    ...
    <>
      <nav className="container mx-auto flex items-center justify-between py-4 px-6">
        ...
        <div className="flex items-center space-x-4">
          {user && user.role==="admin"&&(<Link to='/admin' className="bg-black block px-2 rounded text-white">Admin</Link>)}
          ...
          <button className="relative hover:text-black">
            ...
            />
            {cartItemCount > 0 &&(<span className="absolute -top-1 bg-Rabit-red text-white text-xs rounded-full px-2 py-0.5">
              {cartItemCount}
            </span>)}
      ...
      <div className="p-4">
        ...
        <nav className="space-y-4">
            ...
            <Link to="/collections/all?category=Top Wears" onClick={toggleNavDrawer} className="block text-gray-600 hover:text-black">
                Top Wears
            </Link>
            <Link to="/collections/all?category=Bottom Wears" onClick={toggleNavDrawer} className="block text-gray-600 hover:text-black">
                Bottom Wears
            </Link>
        </nav>
      </div>
